...
def to_stored_value(value: Any) -> Any:
    ...
    share an offset, and UTC is what entities default to.
    """
    if isinstance(value, (list, tuple, set)):
        return type(value)(to_stored_value(v) for v in value)
...
def to_stored_specification(
    ...
    if isinstance(specification, FieldValueSpecification):
        ...
        stored.value = to_stored_value(specification.value)
        return stored
    if isinstance(specification, CollectionSpecification):
        return type(specification)(
            [to_stored_specification(s) for s in specification.specifications]
        )
    if isinstance(specification, NotSpecification):
        return NotSpecification(to_stored_specification(specification.specification))
